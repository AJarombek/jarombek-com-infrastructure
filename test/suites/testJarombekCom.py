...
class TestJarombekCom(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if prod_env:
            ...
            self.lb_certs = [
                ...
                if cert.get("DomainName") == self.cert_url
    ...
    def test_www_jarombek_com_cname_record_exists(self) -> None:
        ...
        try:
            a_record = Route53.get_record(
                "jarombek.com.", f"www.{self.website_url}", "CNAME"
            )
        except IndexError:
            a_record = None
            self.assertFalse(True)
    ...
    def test_listener_http(self) -> None:
        ...
        self.assertEqual(
            default_action.get("RedirectConfig").get("StatusCode"), "HTTP_301"
        )
    ...
    def test_listener_https(self) -> None:
        ...
        https_listeners = [
            item for item in listeners if item.get("Protocol") == "HTTPS"
        ]
        ...
        self.assertEqual(
            default_action.get("TargetGroupArn"), target_group.get("TargetGroupArn")
        )
    ...
    def test_target_group(self) -> None:
        ...
        self.assertEqual(
            target_group.get("TargetGroupName"), f"jarombek-com-{self.env}-lb-target"
        )
    ...
    def test_listener_https_certificate(self) -> None:
        ...
        cert = certs[0]
        self.assertEqual(
            cert.get("CertificateArn"), self.lb_certs[1].get("CertificateArn")
        )

        cert = certs[1]
        self.assertEqual(
            cert.get("CertificateArn"), self.lb_certs[0].get("CertificateArn")
        )
    ...
    def test_lb_security_group_exists(self) -> None:
        ...
        """
        security_groups = SecurityGroup.get_security_groups(
            name=f"jarombek-com-{self.env}-lb-security-group"
        )
        self.assertEqual(len(security_groups), 1)

        sg = security_groups[0]
        self.assertEqual(
            sg.get("GroupName"), f"jarombek-com-{self.env}-lb-security-group"
        )
    ...
    def test_ecs_cluster_running(self) -> None:
        ...
        self.assertEqual(
            cluster.get("clusterName"), f"jarombek-com-{self.env}-ecs-cluster"
        )
    ...
    def test_ecs_task_running(self) -> None:
        ...
        jarombek_com_database_container = [
            container
            for container in containers
            if container.get("name") == "jarombek-com-database"
        ][0]
        self.assertEqual(
            jarombek_com_database_container.get("name"), "jarombek-com-database"
        )
        self.assertEqual(jarombek_com_database_container.get("lastStatus"), "RUNNING")

        jarombek_com_container = [
            container
            for container in containers
            if container.get("name") == "jarombek-com"
        ][0]
    ...
    def test_ecs_service_running(self) -> None:
        ...
        service = services[0]
        self.assertEqual(
            service.get("serviceName"), f"jarombek-com-ecs-{self.env}-service"
        )
    ...
    @unittest.skip("jarombek.com ECS infrastructure no longer used")
    def test_sg_rule_cidr(
        self, rule: dict, protocol: str, from_port: int, to_port: int, cidr: str
    ) -> None:
        """
